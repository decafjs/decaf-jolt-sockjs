...
var Session = require('decaf-jolt-sockjs/lib/Session');
var {SessionWebSocket, SessionEventSource, SessionXhr, SessionXhrStreaming} = Session;
...
function SockJS(app, endpoint, options) {
    ...
    app.verb(endpoint, function (req, res) {
        ...
        if (!req.args.length) {
            ...
            res.end(JSON.stringify(decaf.extend({
                websocket     : true,
                origins       : ['*:*'],
                cookie_needed : false,
                entropy       : random32()
