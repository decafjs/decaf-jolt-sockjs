 ...
 * Updated by alexandrulazar on 3/11/15
 */

var Thread = require('Threads').Thread;

/* global decaf, require, exports, module */
...
function EventBus(sockjs) {
        ...
        sock.on('message', function (message) {
            ...
            switch (message.command) {
                ...
                case 'publish':
                    ...
                    me.publish(message.address, message.payload, message.sock);
                    me.onPublish(message);
                    break;
                case 'send':
                    me.fire('message', message);
                    me.send(message.address, message.payload, message.sock);
                    me.onSend(message);
                    break;
                default:
    ...
    decaf.extend(me, {
         ...
         */
        publish            : sync(function (address, message, socket) {
            var me = this;
                ...
                sync(function () {
                    ...
                    decaf.each(me.subscriptions[address], function (sock) {
                        try {
                            ...
                            if (sock !== socket) {    // don't send to originator of message
         ...
         */
        send               : sync(function (address, message, scoket) {
            var me = this,
                sock;
            ...
            } while (sock === message.sock && me.subscriptions[address].length !== 1);
        }, me.subscriptions)
    });
