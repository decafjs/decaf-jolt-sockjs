 ...
 * Updated by alexlazar on 3/11/15
 */
        ...
        sock.on('message', function (message) {
            ...
            try {
                ...
                /*message = {
                 address: 'ping',
                 command: 'send',
                 payload: message // if text for keeping connection alive
                 }*/
            }
            message.sock = sock;
            switch (message.command) {
                ...
                case 'publish':
                    ...
                    me.publish(message.address, message);
                    me.onPublish(message);
                    break;
                case 'send':
                    me.fire('message', message);
                    me.send(message.address, message);
                    me.onSend(message);
                    break;
                default:
    ...
    decaf.extend(me, {
            ...
            new Thread(function () {
                sync(function () {
                    decaf.each(me.subscriptions[address], function (sock) {
                        try {
                            if (sock !== message.sock) {    // don't send to originator of message!
        ...
        send               : sync(function (address, message) {
            ...
            do {
                ...
                if (sock) {
                    me.subscriptions[address].push(sock);
                    if (sock !== message.sock) {
                        try {
                            if (sock !== message.sock) {    // don't send to
